...
import { Send, Sparkles, Users, FolderKanban, Loader2 } from "lucide-react";
import { format } from "date-fns";
import { createPageUrl } from "@/utils";

export default function ProjectChatDialog({ open, onClose, project, contacts }) {
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Message.filter({ project_id: project.id }, '-created_date'),
    enabled: !!project?.id,
  });
  ...
  const sendMessage = useMutation({
    ...
    onSuccess: async (newMessage) => {
      ...
      // Send notifications to all project collaborators except sender
      if (!newMessage.is_ai_message && project?.collaborators) {
          ...
          if (contact && contact.user_id) {
            try {
              await base44.entities.Notification.create({
                ...
                action_url: createPageUrl('Projects')
              });
            } catch (error) {
  ...
  const handleAskAI = async () => {
    if (!project || !project.ai_assistant_enabled) return;
    ...
    try {
      ...
      const aiResponse = await base44.integrations.Core.InvokeLLM({
        prompt: `You are an intelligent AI assistant for a project management team. You're observing a conversation about this project:
...
INSTRUCTIONS:
1. Analyze what the team is discussing
2. Understand the context, problems, or questions
3. Provide helpful, specific advice based on their actual conversation
4. If they're discussing technical approaches, offer insights on best practices
5. If they're stuck on decisions, help them evaluate options
6. If they're planning, suggest considerations they might have missed
7. Be concise but actionable - focus on what they're ACTUALLY talking about

Don't give generic project management advice. Respond to their SPECIFIC discussion.`
      });
            ...
            {messages.length === 0 && (
              <div className="text-center py-8">
                <p className="text-gray-500 dark:text-gray-400 text-sm">
                  No messages yet. Start collaborating on this project!
                </p>
                {project?.ai_assistant_enabled && (
